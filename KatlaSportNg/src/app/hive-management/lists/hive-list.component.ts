...
export class HiveListComponent implements OnInit {
  ...
  onDelete(hiveId: number) {
    ...
    this.hiveService.setHiveStatus(hive.id, true).subscribe(c => hive.isDeleted = true);
  }

  onRestore(hiveId: number) {
    var hive = this.hives.find(h => h.id == hiveId);
    this.hiveService.setHiveStatus(hive.id, false).subscribe(c => hive.isDeleted = false);
  }
}
