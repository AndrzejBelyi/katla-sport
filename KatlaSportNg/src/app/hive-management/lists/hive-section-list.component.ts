...
export class HiveSectionListComponent implements OnInit {
  ...
  onDelete(hiveSectionId: number) {
    ...
    this.hiveSectionService.setHiveSectionStatus(hiveSection.id, true).subscribe(c => hiveSection.isDeleted = true);
  }

  onUndelete(hiveSectionId: number) {
    var hiveSection = this.hiveSections.find(h => h.id == hiveSectionId);
    this.hiveSectionService.setHiveSectionStatus(hiveSection.id, false).subscribe(c => hiveSection.isDeleted = false)
  }
}
